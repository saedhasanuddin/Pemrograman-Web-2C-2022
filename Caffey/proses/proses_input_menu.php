...
if($cek === false){
    ...
    if(file_exists($target_file)){
        ...
        if($_FILES['foto']['size'] > 500000){
            $message = "file upload foto terlalu besar";
            $statusupload = 0;
        }else{
            if($imagetype !="jpg" && $imagetype !="png" && $imagetype !="jpeg" && $imagetype !="gif"){
                $message = "maaf hanya diperbolehkan yang memiliki format JPG, JPEG, PNG, dan GIF";
                $statusupload = 0;
            }
        }
    }
...
if($statusupload == 0 ){ 
    ...
    if (mysqli_num_rows($select) > 0) {
        $message = '<script>alert("Menu yang dimasukkan telah ada");
                    window.location="../menu"</script>';
    }else{
        if(move_uploaded_file($_FILES['foto']['tmp_name'],$target_file)){
            ...
            $message = '<script>alert("Maaf tejadi kesalahan file tidak dapat di upload");
                        window.location="../menu"</script>';
        }
    }
}
